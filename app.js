...
function call_astro(firstName, lastName, astroId) {
  ...
  if (!loggedIn) {
    ...
    alert(
      "Your Wallet balance is less than INR 100. You need to recharge in order to talk to " +
        firstName +
        " " +
        lastName
    );
...
function follow_astro(astroId) {
  $("#callastroid").val(astroId);
  if (!loggedIn) {
    $("#loginModal").modal();
  } else {
    document.location.href = "/astro-follow/?astroid=" + astroId;
  }
}
